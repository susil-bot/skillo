...
// Tenant = multi-tenant boundary in Skillo (maps to Vendure's Channel under the hood)
const VENDURE_TENANT_TOKEN =
...
interface VendureRequestOptions {
    ...
    /** Tenant token (multi-tenant scope). Maps to Vendure's channel token. */
    tenantToken?: string;
    /** @deprecated Use tenantToken. Kept for backward compatibility. */
    channelToken?: string;
    fetch?: RequestInit;
    tags?: string[];
}
...
export async function query<TResult, TVariables>(
    ...
    // Set tenant token (Vendure API expects it as vendure-token header; 1 tenant = 1 Channel in Vendure)
    headers[VENDURE_CHANNEL_TOKEN_HEADER] = tenantToken ?? channelToken ?? VENDURE_TENANT_TOKEN;
