 ...
 * Redirect to Facebook Login; exchange code for short-lived token, then long-lived.
 * Long-lived tokens last ~60 days; no refresh_token – user re-auth when expired.
 */
...
/**
 * Exchange authorization code for short-lived access token, then long-lived.
 */
async function exchangeCodeForTokens(code, userId = 'default') {
  ...
  // Exchange short-lived for long-lived (~60 days) if we have a short-lived token
  if (data.expires_in) {
...
/**
 * Return stored tokens for user; refresh not supported by Meta (re-auth when expired).
 */
